<script lang="ts">
	...
	onMount(() => {
		if (locales.includes($page.url.pathname.split('/')[1] as Locales) && user.length > 0 && user !== '') {
	...
	const handleInput = (event: { target: { value: string } }) => {
		...
		filteredLocales = locales.filter((loc) => loc.toLowerCase().includes(searchQuery.toLowerCase()));
	};
</script>

<svelte:window on:popstate={handlePopStateEvent} />

<button class="btn btn-sm rounded-full border border-white variant-filled-surface justify-between uppercase" use:popup={languageSettings}>
	{LanguageLabel}
</button>
